...
## Recorded-control pilot, September15 13:37–13:49 UTC

The proposed intervention has now been attempted as a **recorded-action replay pilot**, not live policy inference or exact saved-state branching. Nine75-step runs executed675steps on an A6000. [Results](results/croot005-replays.json) include all failures. The learner replay grasped at23 only; its repeat never grasped. Teacher replay grasped continuously23–46. Base replacement23–26 grasped only at23 in both runs. Arm replacement held longer in one run, but the repeat grasped only23–25. Gripper replacement never grasped. Replacing all actions23–26 grasped only23–26.

This does not establish any replacement as a fix. Learner robot qpos/qvel matched the original through pre-action22 (qpos difference below3e-8), then contact dynamics diverged; complete object/contact state equality was not verified. OriginalC12 had no grasp, while its replay transiently passed the native force/angle test. Recorded future actions also do not adapt to intervention-induced state changes. Therefore these results identify fragile contact and retention as important diagnostic targets but do not isolate an originalC12 root cause. The intended identical-full-state causal experiment remains outstanding.

All replay logs, both finger-force vectors/directions, TCP/object poses and videos were downloaded and SHA256 verified before the temporaryPod was deleted and its absence confirmed. No externalAPI or live model inference ran in this pilot. Native benchmark criteria were unchanged. None of these trajectories counts as C success.
