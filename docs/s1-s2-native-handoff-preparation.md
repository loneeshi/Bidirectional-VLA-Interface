...
> 最新状态（2026-09-17 18:26 EDT）：S1 训练及下方命令的固定十起点评估均已结束，结果 **0/10**。不要重复启动下方历史命令。失败归因、教师观测动作核查和校验修复见 [S1 失败分析](s1-native-failure-analysis.md)。S2 能力门仍未通过。
...
以下为已于 17:31 EDT 启动并完成的历史命令，保留用于复现说明，不能原样再次写入 run01。新批次须独立路径和预算登记；默认去掉 `--execute` 仅CPU预检。运行上限一小时，每episode最多600秒；遇基础设施失败停止并保留目录，不覆盖失败。
