...
def main():
    ...
    p.add_argument('--family',choices=['reach','grasp','move'],required=True)
    ...
    recording = out / f'fetch-s1-ia-{a.family}-seed{a.seed}-episode000-incomplete.mp4'
    ...
    try:
        ...
        from bvi.ia_call_predicates import INSTRUCTIONS,LIMITS,complete,distance
        ...
        report.update(instruction=prompt, instruction_source='frozen_IA_window_template',
        ...
        with (out / 'events.jsonl').open('x') as log:
            for step in range(LIMITS[a.family]):
                ...
                report.update(steps=step + 1, success=complete(a.family,jsonable(obs['extra']),jsonable(info),grasp_streak),native_pick_success=bool(info['success'].item()),
                ...
                if report['success']:
                    report['stop_reason']='invocation_predicate';break
                ...
                report['stop_reason'] = 'invocation_action_budget'
        ...
        if report['status'] == 'episode_completed' and recording.exists():
            name = f'fetch-s1-ia-{a.family}-seed{a.seed}-episode000' + ('' if report['success'] else '-failed') + '.mp4'
