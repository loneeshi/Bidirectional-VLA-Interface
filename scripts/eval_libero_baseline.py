...
def axis_angle(q):
    ...
    return np.zeros(3) if den < 1e-8 else q[:3] * (2.0 * np.arccos(q[3]) / den)
    ...
    def rgb(k):
        return image_tools.convert_to_uint8(
            image_tools.resize_with_pad(
                np.ascontiguousarray(obs[k][::-1, ::-1]), 224, 224
            )
        )

    return {
        "observation/image": rgb("agentview_image"),
        "observation/wrist_image": rgb("robot0_eye_in_hand_image"),
        "observation/state": np.concatenate(
            (
                obs["robot0_eef_pos"],
                axis_angle(obs["robot0_eef_quat"]),
