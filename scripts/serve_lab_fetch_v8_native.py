"""Frozen original V8 pi0.5 Unix-socket server; no progress head or training.

Protocol: send {'op':'reset','seed':N}; then {'op':'predict','workspace_rgb':...,
'wrist_rgb':...,'state':state30,'prompt':str,'seed':N}; finally {'op':'shutdown'}.
Client EOF clears episode RNG and blocks for the next authenticated connection.
Each predict splits the episode JAX key once. Images are uint8; state30 is raw
qpos+qvel with XY relative to the original native episode start. Returned actions
are unnormalized Fetch13, horizon10, with no clipping/head mask added here.
"""
import argparse
import dataclasses
import hashlib
...
def main():
    ...
    p.add_argument('--max-inference-calls', type=int, default=1200)
    a = p.parse_args()
    if not 1 <= a.max_inference_calls <= 1200:
        p.error('--max-inference-calls must be in[1,1200]')
    ...
    report = dict(status='loading', training_updates=0, progress_head=False, tapt=False,
                  ...
                  action_postprocessing='V8 quantile unnormalization only; client owns clipping/headmask',
    ...
    try:
        ...
        cfg = fetch_config(a.repo_id, str(output / 'unused'), state_input=True,
        ...
        if set(flat) != set(ref) or len(flat) != 71 or any(v.shape != ref[k].shape for k, v in flat.items()):
            raise ValueError('Strict originalV8 checkpoint mismatch: expected71 exact key/shape matches')
        if any('progress_chunk' in str(k) for k in flat):
            raise ValueError('Original frozenV8 must have no progress head')
        ...
        for key, dim in [('state', 30), ('actions', 13)]:
            for quantile in ['q01', 'q99']:
                ...
                if values.shape != (dim,) or not np.isfinite(values).all():
                    raise ValueError('Invalid V8 quantile normalization statistics')
        ...
        while True:
            ...
            try:
                ...
                workspace, wrist = np.asarray(request['workspace_rgb']), np.asarray(request['wrist_rgb'])
                state = np.asarray(request['state'], np.float32)
                prompt = request['prompt']
                if workspace.shape != (224, 224, 3) or wrist.shape != (128, 128, 3) or workspace.dtype != np.uint8 or wrist.dtype != np.uint8:
                    raise ValueError('Invalid V8 workspace/wrist RGB contract')
                if state.shape != (30,) or not np.isfinite(state).all() or not isinstance(prompt, str) or not prompt.strip():
                    raise ValueError('Finite original-start-relative state30 and nonempty prompt required')
                t0 = time.monotonic()
                ...
                if actions13.shape != (10, 13) or not np.isfinite(actions13).all():
                    raise ValueError('Nonfinite/invalid V8 action output')
