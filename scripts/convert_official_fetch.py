"""CPU official H5 -> split LeRobot datasets and S2 interval evidence. No training."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    p.add_argument('--scale',choices=['pilot','medium','full'],default='pilot')
    p.add_argument('--task',choices=['pick','place'],default='pick')
    ...
    report=dict(status='checking_source',question='S1 official native RGB/state/action alignment and S2 current-observation interval preparation',
        task=a.task,scale=a.scale,gpu_runs=0,api_calls=0,training_updates=0,episodes=[],
        state_dim=24,state_components=['native_qpos12','native_qvel12'],
        ...
        annotation_only_keys=['is_grasped','tcp_pose_wrt_base','obj_pose_wrt_base','goal_pos_wrt_base'],
        ...
        with h5py.File(source) as h:
            for split,ids in report['parent_split'].items():
                ...
                if not a.audit_only:
                    ...
                    dataset=LeRobotDataset.create(repo_id=f'bvi/s1-official-{a.task}-{a.scale}-{split}',root=a.output/split,
                        ...
                        features={'image':{'dtype':'image','shape':(128,128,3),'names':['height','width','channel']},
                            ...
                            'state':{'dtype':'float32','shape':(24,),'names':['state']},
