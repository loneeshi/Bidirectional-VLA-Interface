"""S1-IA oracle-assisted native Pick; causal training-window prompts,200 total actions.

Fresh native initial states only; no SAC actions, progress head, or success override.
Derived from eval_native_s1.py; the historical fixed-sentence entry remains unchanged.
"""
import argparse
...
def main():
    ...
    p.add_argument('--shader', choices=['minimal'], default='minimal')
    p.add_argument('--sim-backend', choices=['gpu'], default='gpu')
    p.add_argument('--reference-state', type=Path)
    a = p.parse_args()
    if a.reference_state is not None and a.reference_state.name != 'initial-state.pt':
        p.error('Only a recorded native initial-state.pt may be paired; no SAC call-state inputs')
    if a.seed not in (2024, 2025, 2026, 2027, 2028):
    ...
    report = dict(status='preflight', seed=a.seed, scene_split='val',
        ...
        policy='S1-IA oracle-assisted native invocation sequence; no progress or family banks',
        native_predicate='grasped + ee_rest<=0.05m + robot_rest + is_static + cumulative_force<5000',
        precision='bfloat16_model_compute; checkpoint_parameter_dtypes_preserved',
        predicted_action_horizon=10, executed_per_prediction=1,
    ...
    recording = out / f'fetch-s1-ia-oracle-pick-seed{a.seed}-episode000-incomplete.mp4'
    ...
    try:
        ...
        from bvi.s1_oracle_protocol import PickOracle, PROTOCOL
        ...
        if bool(obs['extra']['is_grasped'].item()) or bool(info['success'].item()) or bool(info['fail'].item()):
            raise ValueError('Native initial state must be unheld and nonterminal')
        # Preserve the pre-action TCP/target observation for first-action diagnostics.
        report['initial_extra'] = jsonable(obs['extra'])
        report['initial_native_info'] = jsonable(info)
        save()
        oracle = PickOracle(previous_held=False)
        prompt = oracle.instruction
        report.update(instruction_protocol=PROTOCOL, policy='S1-IA oracle-assisted native invocation sequence',
                      feedback='simulator predicates; no learned progress or GPT',
                      queue_policy='first action only; discard remaining nine at every prediction',
                      prompt_switches=[])
        report.update(instruction=prompt, instruction_source='frozen_IA_training_window_templates',
        ...
        if (contract.get('state_dim') != 24 or contract.get('base_camera') != 'fetch_head'
                or contract.get('training_stage') != 'S1_IA_single_bank_no_progress'):
        ...
        with (out / 'events.jsonl').open('x') as log:
            for step in range(200):
                ...
                switch = oracle.after_action(distance(jsonable(obs['extra'])), held)
                if switch is not None:
                    report['prompt_switches'].append(switch)
                report.update(final_instruction_family=oracle.family, dropped_after_hold=oracle.dropped_after_hold)
                ...
                log.write(json.dumps(jsonable(dict(step=step + 1, raw_actions=actions, action=applied,
                    instruction=prompt, instruction_family=used_family, prompt_switch=switch,
                    consumed_chunk_index=0, discarded_chunk_entries=9,
        ...
        if report['status'] == 'episode_completed' and recording.exists():
            name = f'fetch-s1-ia-oracle-pick-seed{a.seed}-episode000' + ('' if report['success'] else '-failed') + '.mp4'
