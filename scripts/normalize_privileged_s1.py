...
The resulting raw privileged state42/action13 statistics are applied before model padding.
"""
import argparse
import hashlib
import inspect
...
def main():
    ...
    if manifest['status'] != 'converted_not_training_validated' or manifest['state_dim'] != 42 or manifest.get('policy_state_schema') != 'official_h5_fetch_agent24_extra18_v1':
        raise ValueError('Expected completed privileged42 export')
    ...
    report = dict(status='train_only_stats_ready_not_runtime_validated', frames=frames,
                  ...
                  dimensions=dict(state=42, actions=13),
                  policy_state_schema='official_h5_fetch_agent24_extra18_v1', privileged_policy_inputs=True, delta_transform=False,
