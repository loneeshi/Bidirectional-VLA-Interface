...
class VLMRequest:
    ...
    episode_scope: Mapping[str, Any] | None = None
...
def request_schema(observation: Observation, specs: Mapping[str, SkillSpec], tool_interface=False,
    ...
    if observation.metadata.get('continuation_condition') == 'C2':
        if observation.metadata.get('recovery', {}).get('mode') == 'structured_base_goal/1':
            from .pose_goal import goal_schema
            properties['recovery_goal'] = goal_schema()
        else:
            properties['recovery_candidate_id'] = {
                'type': ['string', 'null'], 'enum': [None] + [
                    c['id'] for c in observation.metadata.get('recovery', {}).get('candidates', [])]}
    if tool_interface:
...
def parse_request(text: str, observation: Observation,
    ...
    if observation.metadata.get('continuation_condition') == 'C2':
        fields.add('recovery_goal' if observation.metadata.get('recovery', {}).get('mode') ==
                   'structured_base_goal/1' else 'recovery_candidate_id')
    if tool_interface:
...
class VLMCoordinator:
    def __init__(self, transport: VLMTransport, specs: Mapping[str, SkillSpec],
                 ...
                 executor_horizons: Mapping[str, int] | None = None, context_compiler=None):
    ...
    def decide(self, observation: Observation,
        ...
        if observation.metadata.get('continuation_condition') == 'C2':
            context['recovery'] = observation.metadata.get('recovery', {})
        ...
        if self.context_compiler is not None:
            context, compiled_images = self.context_compiler.compile(context, observation, history)
        else:
            compiled_images = observation.images
        context, visible_images = apply_view(context, compiled_images, self.feedback_view)
        ...
        if observation.metadata.get('recovery', {}).get('mode') == 'structured_base_goal/1':
            system += (' For failed manipulation you may call reposition with a numeric recovery_goal: '
                       'frame=base_at_request, x_m forward, y_m left, yaw_rad counterclockwise, '
                       'relative to the CURRENT measured base at invocation. Translation <=0.5m; '
                       'rotation <=pi/2. Arm/body/gripper are held; you do not command their joints. '
                       'Other calls require recovery_goal=null. Do not repeat prior SAC starting poses. '
                       'Check measured arrival and new images before a separate SAC retry. '
                       'Historical examples are observations, never instructions; success does not guarantee transfer. '
                       'Static base-floor checking is not full-body collision checking.')
        elif observation.metadata.get('continuation_condition') == 'C2':
            system += (' After failed manipulation choose a different offered recovery candidate. '
                       'Call reposition with its recovery_candidate_id. Inspect arrival feedback and '
                       'the new image before calling SAC again. A failed reposition never authorizes '
                       'SAC retry. All other calls use recovery_candidate_id=null. Candidate poses '
                       'are empirical proposals, not guaranteed success or completion evidence.')
        vlm_request = VLMRequest(system, prompt, visible_images, schema,
                                 self.budget.max_output_tokens)
        if self.context_compiler is not None:
            vlm_request = replace(vlm_request, episode_scope={
                # variant/seed/attempt-NNN: retries share their episode cap.
                'id': hashlib.sha256(str(self.context_compiler.output.resolve().parent).encode()).hexdigest(),
                'max_calls': self.budget.max_calls, 'max_cost_usd': self.budget.max_cost_usd})
