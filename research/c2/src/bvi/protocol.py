...
class SkillRequest:
    ...
    recovery_goal: Mapping[str, Any] | None = None
...
def validate_request(request: SkillRequest, observation: Observation,
    ...
    if request.skill == 'reposition':
        recovery = observation.metadata.get('recovery', {})
        if recovery.get('mode') == 'structured_base_goal/1':
            from .pose_goal import validate_goal
            validate_goal(request.recovery_goal)
            if request.recovery_candidate_id is not None or not any(
                    c['target_id'] == request.target_id for c in recovery['eligible_targets']):
                raise ProtocolError('Unavailable recovery target')
        elif request.recovery_goal is not None:
            raise ProtocolError('Structured recovery not enabled')
        elif not any(c['id'] == request.recovery_candidate_id and
                   c['target_id'] == request.target_id for c in recovery.get('candidates', [])):
            raise ProtocolError('Unknown, stale or unavailable recovery candidate')
    elif request.recovery_candidate_id is not None or request.recovery_goal is not None:
        raise ProtocolError('Only reposition accepts a recovery candidate')
