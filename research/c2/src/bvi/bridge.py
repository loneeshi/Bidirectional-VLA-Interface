...
from .coordinator import APIBudget, APIBudgetExhausted, VLMRequest, VLMResponse
...
def encode_request(request: VLMRequest, provider: str, model: str,
    ...
    envelope = {"version": 1, "bridge_id": attempt_id, "provider": provider, "model": model,
    ...
    if request.episode_scope is not None:
        envelope['episode_scope'] = dict(request.episode_scope)
    return envelope
...
def decode_request(envelope: dict[str, Any]) -> VLMRequest:
    ...
    image_cap = 5 if 'recovery_goal' in data['schema'].get('properties', {}) else 2
    if not 1 <= len(data.get("images", [])) <= image_cap:
    ...
    for frame in images:
        frame.validate()
    scope=envelope.get('episode_scope')
    if scope is not None:
        if (not isinstance(scope,dict) or set(scope)!={'id','max_calls','max_cost_usd'}
                or not isinstance(scope['id'],str) or not re.fullmatch(r'[0-9a-f]{64}',scope['id'])
                or type(scope['max_calls']) is not int or not 1<=scope['max_calls']<=120
                or type(scope['max_cost_usd']) not in (int,float)
                or not math.isfinite(scope['max_cost_usd']) or scope['max_cost_usd']<=0):
            raise ProtocolError('Invalid C2 physical-request episode scope')
    return VLMRequest(data["system"], data["prompt"], images, data["schema"],
                      data["max_output_tokens"], attempt_id, scope)
        ...
        while time.monotonic() < deadline:
            ...
            if response_path.is_file():
                ...
                if not response.get("ok"):
                    if request.episode_scope and response.get('error_type')=='APIBudgetExhausted':
                        raise APIBudgetExhausted('Physical-request bridge budget exhausted')
...
class BridgeProcessor:
    ...
    def episode_exhausted(self, relevant, scope):
        if scope is None: return False
        rows=[r for r in relevant if r.get('episode_scope',{}).get('id')==scope['id']]
        if any(r['episode_scope']!=scope for r in rows):
            raise ProtocolError('Episode budget changed after its first physical request')
        return (len(rows)>=scope['max_calls'] or
                sum(r['reserved_cost_usd'] for r in rows)+self.budget.request_cost_ceiling_usd
                > scope['max_cost_usd']+1e-12)
    ...
    def process(self, envelope: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.episode_exhausted(relevant,request.episode_scope):
            raise APIBudgetExhausted('Physical-request episode budget exhausted')
        if (len(relevant) >= self.budget.max_calls
                or reserved + self.budget.request_cost_ceiling_usd > self.budget.max_cost_usd + 1e-12):
            if request.episode_scope:raise APIBudgetExhausted('Local bridge experiment budget exhausted')
        ...
        if request.episode_scope is not None:
            record['episode_scope']=dict(request.episode_scope)
        ...
        for provider_attempt in range(self.max_provider_retries + 1):
            if provider_attempt:
                ...
                relevant = [item for item in claims if item["authorization_id"] == self.budget.authorization_id]
                reserved = sum(item["reserved_cost_usd"] for item in relevant)
                if (self.episode_exhausted(relevant,request.episode_scope) or
                        len(relevant) >= self.budget.max_calls or reserved + self.budget.request_cost_ceiling_usd
