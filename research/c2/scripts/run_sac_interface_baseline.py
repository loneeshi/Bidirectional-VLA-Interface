...
GPU = os.environ.get('BVI_GPU', '')
OFFICIAL_MSHAB = Path(os.environ.get('BVI_MSHAB_ROOT', '/opt/mshab'))


def runtime_environment(root: Path) -> dict[str, str]:
    """Pin the official 42D runtime ahead of the acdit venv's editable fork."""
    root = root.resolve()
    if not GPU or not os.environ.get('MS_ASSET_DIR'):
        raise ValueError('Set BVI_GPU and MS_ASSET_DIR explicitly before execution')
