...
def main() -> None:
    ...
    parser.add_argument('--c2-feedback', choices=['basic','trace','experience'])
    parser.add_argument('--c2-case-bank', type=Path)
    parser.add_argument('--continuation-condition', choices=['C0','C2'],
    ...
    if args.c2_feedback and (args.continuation_condition != 'C2' or args.feedback_profile != 'raw_v0'
                            or args.spawn_prior or args.hide_feedback or not args.evaluation_manifest):
        parser.error('C2 feedback study requires C2/raw_v0, frozen evaluation manifest and no legacy prior/views')
    if bool(args.c2_case_bank) != (args.c2_feedback == 'experience'):
        parser.error('Only C2-experience requires a case bank')
    c2_bank = None
    if args.c2_feedback:
        from bvi.feedback.c2_context import CaseBank
        c2_rows = json.loads(args.evaluation_manifest.read_text(encoding='utf-8'))['episodes']
        if len(c2_rows) != 3 or len({r['plan_uid'] for r in c2_rows}) != 3:
            parser.error('C2 study must bind exactly three distinct development plans')
        matched = [r for r in c2_rows if r['seed']==args.seed and r['plan_uid']==args.expected_plan_uid]
        if len(matched)!=1 or matched[0]['initial_state_sha256'] != args.expected_initial_state_sha256:
            parser.error('C2 seed/plan/initial-state binding mismatch')
        if args.c2_case_bank:
            c2_bank=CaseBank(args.c2_case_bank,{r['plan_uid'] for r in c2_rows})
    ...
    if args.continuation_condition:
        ...
        if args.continuation_condition == 'C2' and not args.c2_feedback:
            ...
            if not audit['ready']:
                parser.error('C2 pose recovery blocked: ' + ','.join(audit['reasons']))
    ...
    if args.paired_ppo_episode and (args.benchmark_episode or args.tool_family_interface
            ...
            or args.max_env_steps != 7000 or args.max_wall_seconds != (1800 if args.c2_feedback else 900)
            ...
            or (not args.dry_run and (not args.organizer or args.model != 'gpt-5.6-luna'
                ...
                or args.max_calls != (90 if args.c2_feedback else 24 if args.continuation_condition == 'C0' else 40)
    ...
    metadata['c2_feedback'] = {'protocol':'c2-pose-feedback/3','variant':args.c2_feedback,
                             'case_bank_sha256': c2_bank.sha256 if c2_bank else None}
    ...
    try:
        ...
        if args.goal_tools:
            ...
            if args.continuation_condition:
                ...
                adapter = ContinuationGoalAdapter(adapter, args.continuation_condition,
                    prior['data'] if prior is not None else None, structured_recovery=bool(args.c2_feedback))
                if args.continuation_condition == 'C2':
                    from bvi.pose_recovery import FetchPosePreparation, RepositionSkill
                    if args.c2_feedback:
                        from bvi.pose_goal import StaticFloorValidator, GoalFetchPreparation, GoalRepositionSkill
                        adapter.recovery.backend = GoalFetchPreparation(adapter,StaticFloorValidator(adapter,ASSET_DIR))
                        RepositionSkill = GoalRepositionSkill
                    else:
                        validator = getattr(adapter.base, 'validate_recovery_motion', None)
                        if not callable(validator):
                            raise ProtocolError('C2 blocked: live scene motion validator is unavailable')
                        adapter.recovery.backend = FetchPosePreparation(adapter, validator)
            ...
            if args.continuation_condition == 'C2':
                from bvi.protocol import SkillSpec
                specs['reposition'] = SkillSpec('reposition', ('pose_arrived',), ('pose_arrived',),
                                               200, args.skill_wall_seconds)
                skills['reposition'] = RepositionSkill(adapter)
        ...
        if args.organizer:
            ...
            metadata['organizer'] = {'enabled': True, 'scope': 'goal_decomposition' if args.goal_tools else 'benchmark_constrained_execution',
                ...
                'reposition_skill': args.continuation_condition == 'C2', 'abort_is_success': False}
            (args.output / 'run-metadata.json').write_text(json.dumps(metadata, indent=2), encoding='utf-8')
        if args.feedback_profile in ('object_trajectory_v1', 'object_trajectory_prior_v1'):
            ...
            skills = {name: (skill if name == 'reposition' else TrajectorySkill(skill, adapter))
                      for name, skill in skills.items()}
        if args.c2_feedback:
            from bvi.feedback.c2_context import C2ContextCompiler, ContextSkill
            categories={}; checkpoint_hashes={}
            for goal in adapter.catalog.goals:
                category=goal['object_name'].split('-')[0]
                for skill,target in [('pick',goal['object_id']),('place',goal['destination_id'])]:
                    categories[target]=category
                    checkpoint_hashes[(skill,target)]=hashlib.sha256(
                        (checkpoint_base/skill/category/'policy.pt').read_bytes()).hexdigest()
            context_compiler=C2ContextCompiler(args.c2_feedback,args.output,c2_bank,categories,checkpoint_hashes)
            skills={name:ContextSkill(skill,adapter) if name in ('pick','place') else skill
                    for name,skill in skills.items()}
        ...
        for index in range(args.max_calls):
            ...
            if args.continuation_condition and (not args.c2_feedback or result.feedback.status is not SkillStatus.REJECTED):
                adapter.finish_request(request, result.feedback.status is SkillStatus.SUCCEEDED)
            if context_compiler is not None and request.skill in ('pick','place') and result.steps:
                context_compiler.note(request,result,skills[request.skill].trace.finish())
            if organizer:
            ...
            if args.continuation_condition == 'C2':
                # Snapshot: later mutations must not rewrite earlier GPT inputs.
                import copy
                history[-1]['recovery_candidate_id'] = request.recovery_candidate_id
                history[-1]['recovery_goal'] = request.recovery_goal
                history[-1]['pose_recovery'] = copy.deepcopy(adapter.recovery.snapshot())
