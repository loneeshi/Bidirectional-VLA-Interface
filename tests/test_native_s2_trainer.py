...
SCRIPT = Path(__file__).resolve().parents[1] / 'scripts/train_native_s2.py'
...
def dataset(tmp_path):
    ...
    for split in ('train', 'validation'):
        for family in trainer.FAMILIES:
            ...
            np.savez(path, head_rgb=np.zeros((3, 128, 128, 3), np.uint8),
                wrist_rgb=np.zeros((3, 128, 128, 3), np.uint8),
                state=np.zeros((3, 24), np.float32), actions=np.zeros((3, 13), np.float32),
...
def test_training_rejects_missing_s1_before_gpu_or_dataset(tmp_path):
    result = subprocess.run([sys.executable, str(SCRIPT), '--data', str(tmp_path)],
