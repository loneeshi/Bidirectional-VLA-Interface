...
class Backend:
    ...
    def infer(self, observation, **kwargs):
        ...
        return {"actions": [[0.] * 7, [1.] * 7], "progress": [.2, .8], "progress_source": "learned"}
...
class FamilyTests(unittest.TestCase):
    def setUp(self):
        ...
        self.events = []
        self.session = FamilySession(self.backend, lambda e, **kw: self.events.append((e, kw)))
    ...
    def test_instruction_and_family_reach_backend(self):
        ...
        self.session.act({})
        self.assertEqual(self.backend.received, {"instruction": text, "tool_family": "grasp"})
    ...
    def test_call_ids_cannot_be_reused(self):
        ...
        with self.assertRaises(ValueError): self.session.start(call)
