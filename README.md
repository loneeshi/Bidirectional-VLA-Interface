...
**September 16 mainline (latest evidence: September 17 UTC):** manipulation uses **AC-DiT** on the laboratory server. [Fetch TAPT SFT](docs/fetch-tapt-sft-and-online.md) completed 1,199 updates with a frozen native backbone and four trained family residual banks. The first learned-progress online Pick **failed**: premature reach/grasp switching, no object grasp. [Paired progress-path audit](docs/fetch-tapt-progress-attribution.md) found a post-action-label / pre-action-decision mismatch and unsupported handoff states; it did not establish inference feature drift as the main cause. No training job is running. GPT/LightNav integration with AC-DiT, actual navigation-handoff training coverage, DROID and GRPO remain pending. Historical pi05 results below remain separate.
