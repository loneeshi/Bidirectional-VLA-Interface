...
**September 16 mainline (latest evidence: September 17 UTC):** manipulation uses **AC-DiT** on the laboratory server. [Fetch TAPT SFT](docs/fetch-tapt-sft-and-online.md) completed 1,199 updates with the native backbone frozen. [Progress timing fix and paired retest](docs/fetch-progress-timing-fix.md) now preserve the old checkpoint's post-action semantics and record replayable inputs. The old failure reproduced exactly; corrected timing still switched reach at23.44cm and **did not grasp**. Six recorded decisions replayed bitwise-identical actions/progress. Feedback calibration remains unresolved; no new training occurred. GPT/LightNav integration, actual navigation-handoff training coverage, DROID and GRPO remain pending.
