...
class ProgressChunk:
    ...
    def __post_init__(self):
        ...
        if not self.values or any(not math.isfinite(x) or not 0 <= x <= 1 for x in self.values):
...
class FamilySession:
    ...
    def start(self, invocation: FamilyInvocation):
        if self.active is not None:
            raise RuntimeError("Finish or interrupt the active invocation before switching")
        ...
        self.emit("family_selected", call_id=invocation.call_id, family=invocation.tool_family,
                  instruction=invocation.instruction, adapter_sha256=digest, protocol=VERSION)
    ...
    def act(self, observation):
        ...
        if not self.queue:
            try:
                result = self.backend.infer(observation, instruction=self.active.instruction,
                                            tool_family=self.active.tool_family)
                actions = result["actions"]
                if not len(actions) or any(len(a) != 7 or any(not math.isfinite(float(v)) for v in a) for a in actions):
                    raise ValueError("Expected finite LIBERO 7D actions")
                progress = ProgressChunk(self.active.call_id, tuple(result["progress"]), result["progress_source"])
                ...
                self.emit("progress_chunk", call_id=progress.call_id, values=progress.values, source=progress.source)
            except Exception:
    ...
    def finish(self, reason):
        if self.active is not None:
            self.emit("invocation_finished", call_id=self.active.call_id, reason=reason, steps=self.steps)
        self.queue.clear()
        self.active = None
